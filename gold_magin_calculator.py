...
if bonus_enabled:
    bonus_percent = st.slider("Bonus Percentage (%)", min_value=0, max_value=100, value=30)
else:
    bonus_percent = 0
...
ounces_without_leverage = trading_percent * (deposit + deposit * bonus_percent / 100) / 100 / current_price if current_price > 0 else 0
total_balance = deposit + deposit * bonus_percent / 100
...
# State to control calculate button
if 'calculated' not in st.session_state:
    st.session_state.calculated = False

if st.button("Calculate") or st.session_state.calculated:
    ...
    st.markdown(f"Bonus Amount Given: **${deposit * bonus_percent / 100:,.2f}** ({bonus_percent}%)")
